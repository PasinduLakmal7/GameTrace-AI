...
export default function EventsExplorerPage() {
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-in fade-in duration-500">
      ...
      {/* Search and Filters */}
      <div className="bg-zinc-900 border border-zinc-800/60 rounded-t-2xl p-4 flex flex-col sm:flex-row gap-4 items-center justify-between border-b-0">
        <div className="relative w-full max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
          <input 
            type="text" 
            placeholder="Search value or player..."
            className="w-full bg-zinc-800 border border-zinc-800/60 rounded-xl pl-9 pr-4 py-2 text-sm text-zinc-200 outline-none focus:border-orange-500/50 transition-colors"
          />
        </div>
        
        <div className="flex gap-4 items-center">
          <button className="px-4 py-2 bg-zinc-800 border border-zinc-800/60 rounded-xl text-sm text-zinc-300 flex items-center justify-between gap-2 hover:bg-zinc-800/50 transition-colors w-32">
      ...
      {/* Table */}
      <div className="bg-zinc-900 border border-zinc-800/60 rounded-b-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/60 bg-zinc-900">
              <tr>
