...
export default function SessionDetailsPage({ params }: { params: { sessionId: string } }) {
  // In a real app, you would fetch the session using params.sessionId
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className={cn(
              ...
              session.status === "PLAYER_DEAD" ? "bg-rose-500/10 text-rose-400 border-rose-500/20" : 
        ...
        <div className="flex gap-3">
          <Link href={`/dashboard/ai/${session.sessionId}`} className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg text-sm text-zinc-300 flex items-center gap-2 transition-colors">
            <Sparkles className="w-4 h-4 text-amber-400" /> View AI Report
          </Link>
          <Link href={`/replay/${session.sessionId}`} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm text-white font-medium flex items-center gap-2 transition-colors shadow-lg shadow-indigo-500/20">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {['PLAYER_HIT', 'PLAYER_SHOOT', 'ENEMY_HIT', 'ENEMY_DIE'].map(type => {
